"""
Zalando connect/status/disconnect routes — thin, email-fallback-only scope
(2026-09-15). No /callback route — client_credentials grant, tenant
submits their own client_id/client_secret directly, same shape as this
repo's Walmart connector. No webhook route.
"""
...
router = APIRouter(prefix="/marketplaces/zalando", tags=["marketplaces"])
_ADMIN_ROLES = ("admin", "tenant_admin")


class ZalandoConnectRequest(BaseModel):
    client_id: str
    client_secret: str
...
@router.post("/connect")
async def zalando_connect(
    body: ZalandoConnectRequest,
    current_user: CurrentUser = Depends(require_role(*_ADMIN_ROLES)),
    db: AsyncSession = Depends(get_db),
) -> dict:
    settings = get_settings()
    if not settings.ZALANDO_ENABLED:
        return {"error": "Zalando integration is not enabled on this deployment"}

    result = await zalando_connector.connect(str(current_user.tenant_id), {
    ...
    existing = (
        await db.execute(
            select(MarketplaceConnection).where(
                ...
                MarketplaceConnection.provider == "zalando",
            )
        )
    ).scalar_one_or_none()
    if existing:
...
async def zalando_connection_status(
    current_user: CurrentUser = Depends(require_role(*_ADMIN_ROLES)),
    db: AsyncSession = Depends(get_db),
) -> dict:
    conn = (
