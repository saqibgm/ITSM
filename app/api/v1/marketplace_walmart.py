"""
Walmart connect/status/disconnect routes — pilot batch #3. No /callback
route (unlike Shopify/Amazon) — Walmart has no OAuth redirect/consent flow;
the tenant submits their own Walmart-issued client_id/client_secret directly
via /connect (see connectors/walmart.py's module docstring for why). No
webhook route either — the signing scheme wasn't confirmed in this org's
research; see connectors/walmart.py's parse_webhook() docstring.
"""
...
router = APIRouter(prefix="/marketplaces/walmart", tags=["marketplaces"])
...
@router.post("/connect")
async def walmart_connect(
    body: WalmartConnectRequest,
    ...
    existing = (
        await db.execute(
            select(MarketplaceConnection).where(
                ...
                MarketplaceConnection.provider == "walmart",
            )
        )
    ).scalar_one_or_none()
    if existing:
...
async def walmart_connection_status(
    current_user: CurrentUser = Depends(require_role(*_ADMIN_ROLES)),
    db: AsyncSession = Depends(get_db),
) -> dict:
    conn = (
